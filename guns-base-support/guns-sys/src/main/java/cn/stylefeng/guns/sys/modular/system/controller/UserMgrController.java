...
public class UserMgrController extends BaseController {
    ...
    @ResponseBody
    public ResponseData upload(@RequestPart("file") MultipartFile picture, HttpServletRequest request) {
        String photoId = (String) request.getSession().getAttribute("photoId");
        String pictureName = photoId + "_" + UUID.randomUUID().toString() + "." + ToolUtil.getFileSuffix(picture.getOriginalFilename());
        try {
            String fileSavePath = ConstantsContext.getFileUploadPath();
            String fileId ="D:\\Java\\Java_Study_Files\\java_note\\Guns\\UserImage\\" + pictureName;
            picture.transferTo(new File(fileId));
//            int i = 1/0;
...
//            throw new ServiceException(BizExceptionEnum.UPLOAD_ERROR);
    ...
    /**
     * 将学生stuNumber存入session
     * @param request
     */
    @RequestMapping(path = "/photoId")
    public void createStuId(@RequestParam("photoId") String photoId,HttpServletRequest request){
